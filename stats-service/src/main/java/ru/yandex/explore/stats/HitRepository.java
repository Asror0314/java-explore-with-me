...
public interface HitRepository extends JpaRepository<Hit, Long> {
    ...
    @Query("SELECT new ru.yandex.explore.stats.dto.StatsDto( h.uri, h.app, COUNT(h.ip) ) " +
            "FROM Hit as h " +
    ...
    @Query("SELECT new ru.yandex.explore.stats.dto.StatsDto( h.uri, h.app, COUNT(distinct h.ip) ) " +
            "FROM Hit as h " +
            "WHERE h.timestamp >= ?1 AND h.timestamp <= ?2 " +
            "AND h.uri in ?3 " +
